...
export const TestModal = observer(() => {
  const { testsDivIsVisible, setTestsDivIsVisible, testsResponse } = storeTests
  if (!testsDivIsVisible) return null
  return (
    <>
      {createPortal(
        <div className="absolute left-0 top-0 z-10 w-full h-full bg-black/80">
          <button
            ...
            onClick={() => setTestsDivIsVisible(false)}
          />
          <div
          ...
          >
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-auto break-all">
              <p className="text-bolder text-xl mb-2 p-2">Tests:</p>
              <p className="text-base text-bolder p-4 whitespace-break-spaces">
                {testsResponse?.map((data) => <p>{String(data)}</p>)}
              </p>
            </div>
          </div>
        </div>,
